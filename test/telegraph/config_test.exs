defmodule Telegraph.ConfigTest do
  use ExUnit.Case
  alias Telegraph.Config

  defp restore_env!(key, value) do
    if value do
      :ok = Application.put_env(:telegraph, key, value)
    else
      :ok = Application.delete_env(:telegraph, key)
    end
  end

  setup do
    base_url = Application.get_env(:telegraph, :base_url)
    graph_base_url = Application.get_env(:telegraph, :graph_base_url)
    file_base_url = Application.get_env(:telegraph, :file_base_url)
  ...
  test "Config.file_base_url/0 returns default when unset" do
    :ok = Application.delete_env(:telegraph, :file_base_url)
